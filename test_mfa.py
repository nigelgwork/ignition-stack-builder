                ...
                try:
                    ...
                    if isinstance(resp_json, dict):
                        ...
                        if 'qr_code' in display_json:
                            display_json['qr_code'] = f"<{len(display_json['qr_code'])} chars>"
                        if 'backup_codes' in display_json:
                            display_json['backup_codes'] = f"<{len(display_json['backup_codes'])} codes>"
...
def main():
    ...
    if verify_final_response.status_code != 200:
        print(f"{RED}Failed to verify MFA for disable test: {verify_final_response.text}{NC}")
        tests_failed += 1
        # Test Summary anyway
