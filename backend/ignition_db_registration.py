...
def test_database_connection(session, connection_name):
    \"\"\"Test a database connection\"\"\"
    log_info(f"Testing connection: {{connection_name}}")

    try:
        response = session.post(
            urljoin(GATEWAY_URL, f"/data/db-connections/{{connection_name}}/test"),
            timeout=30
        )
        ...
        if response.status_code == 200:
            ...
            if result.get("success"):
                log_success(f"✓ Connection test passed: {{connection_name}}")
                return True
            else:
                log_warning(f"Connection test failed: {{result.get('message')}}")
                return False
        else:
            log_warning(f"Could not test connection: {{response.status_code}}")
            return False

    except Exception as e:
        log_warning(f"Exception testing connection: {{e}}")
        return False
...
def main():
    ...
    print("=" * 60)
    print("🗄️  Ignition Database Auto-Registration")
    print("=" * 60)
    print()
    ...
    # Create each database connection
    success_count = 0
    failed_count = 0
    ...
    for db_config in DATABASES:
        if create_database_connection(session, db_config):
            ...
            # test_database_connection(session, db_config["name"])
            success_count += 1
        else:
            failed_count += 1

        print()  # Blank line between connections
...
def generate_ignition_db_readme_section(databases: List[Dict[str, Any]]) -> str:
    ...
    content = """
## 🗄️ Ignition Database Auto-Registration

Your Ignition Gateway has been configured with automatic database registration.
...
- **Database:** `{config.get('database', config.get('sa_password', 'N/A'))}`
...
If you need to re-register databases or add new ones, run:
...
If auto-registration fails, you can manually add connections using these details:
...
**PostgreSQL:**
```
JDBC URL: jdbc:postgresql://{instance_name}:{config.get('port', 5432)}/{config.get('database', 'postgres')}
Driver: org.postgresql.Driver
Username: {config.get('username', 'postgres')}
Password: {config.get('password', 'postgres')}
...
**MySQL/MariaDB:**
```
JDBC URL: jdbc:mysql://{instance_name}:{config.get('port', 3306)}/{config.get('database', 'mysql')}
Driver: com.mysql.cj.jdbc.Driver
Username: {config.get('username', 'root')}
Password: {config.get('password', 'password')}
Validation Query: SELECT 1
...
**MS SQL Server:**
```
JDBC URL: jdbc:sqlserver://{instance_name}:{config.get('port', 1433)}
Driver: com.microsoft.sqlserver.jdbc.SQLServerDriver
Username: {config.get('username', 'sa')}
Password: {config.get('sa_password', 'YourStrong!Passw0rd')}
Validation Query: SELECT 1
