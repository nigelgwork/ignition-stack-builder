...
def generate_grafana_datasources(datasources: List[Dict[str, Any]]) -> str:
    ...
    """
    provisioning = {"apiVersion": 1, "datasources": []}
...
"""
    ...
    for db in databases:
        ...
        if db_type == "postgres":
            ...
            script += (
                f"# JDBC URL: jdbc:postgresql://{instance_name}:{port}/{db_name}\n"
            )
...
def generate_oauth_env_vars(
    ...
    keycloak_base = (
        f"http://{provider}:8080/realms/{realm_name}/protocol/openid-connect"
    )
